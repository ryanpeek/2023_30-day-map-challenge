...
ggplot() +
  ...
  geom_sf(data = osm_scnd_crop, color=alpha("gray50",0.5), lwd=0.5)+
  geom_sf(data = osm_prim_crop, color=alpha("gray30",0.7), lwd=0.7)+
  stat_density2d(data=rat_sf_sel, aes(x=longitude, y=latitude, fill = after_stat(level)), n=200, alpha = .8, geom = "polygon", show.legend = FALSE) + 
  scale_fill_viridis_c() +
  geom_sf(data=sub_sf, aes(color= name), lwd=0.8, lty=2, 
  ...
  theme(
    ...
    plot.caption = element_text(family = "Roboto Slab", size=14)) +
  facet_wrap(.~year, nrow = 2)

ggsave("figs/day17-flow-rats.png", width=11, height=8.5, dpi=300,
       bg="white")
```

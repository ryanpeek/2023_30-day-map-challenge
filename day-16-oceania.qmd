...
stations <- isd_pts[isd_pts$BEGIN <= 20140101 &
                               isd_pts$END >= 20230101, ]
...
mPPT <- ggplot() + geom_col(data=clim_month, aes(x=MONTH, y=PRCP_mean, fill=PRCP_mean), show.legend = T)+
    ...
    coord_polar() + 
    facet_wrap(clim_month$STNID~., nrow = 4)
mPPT
...
# daily mean temp
(dTEMP <- ggplot() + 
    geom_col(data=clim_day, aes(x=YDAY, y=TEMP_mean, fill=TEMP_mean, color=TEMP_mean), show.legend = T)+
    guides(color="none")+
    theme_ft_rc() +
    #theme_minimal() + 
    labs(y="", x="") +
...
siteMap <- ggplot() +
  ...
  geom_label_repel(data=climdata_stations, aes(label=STNID, geometry=geometry), 
...
ggdraw(siteMap) +
  draw_plot(dTEMP, width = 0.45, height = .85, x = 0.67, y = 0)

library(patchwork)
siteMap + dTEMP + plot_layout(widths=c(2,0.7))

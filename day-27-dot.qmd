...
gflw <- ggplot() + 
  ...
  geom_sf(data=nwis_sites_flow, color=alpha("blue", 0.5), size=0.4, pch=16) +
  #geom_sf(data=nwis_sites_flow, color=alpha("darkblue", 0.7), size=0.4, pch=15)# +
  hrbrthemes::theme_ipsum_rc(grid = FALSE, axis = FALSE) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank()) +
  ggspatial::annotation_north_arrow() +
  labs(subtitle = "Dot: Flow Gages")
gflw

gtmp <- ggplot() + 
  geom_sf(data=us, fill=NA, color="gray70", linewidth=0.5, alpha=0.4) +
  geom_sf(data=nwis_sites_temp, color=alpha("maroon", 0.5), size=0.2, pch=16) +
  #geom_sf(data=nwis_sites_flow, color=alpha("darkblue", 0.7), size=0.4, pch=15)# +
  hrbrthemes::theme_ipsum_rc(grid = FALSE, axis = FALSE) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank()) +
  labs(subtitle = "Dot: Temp Gages")
gtmp
  
library(patchwork)

gflw + gtmp + 
  patchwork::plot_annotation(title="Dots: USGS Gages",
                             subtitle = "#30DayMapChallenge • Day-27 • Dot",
                             caption = "R. Peek • Data: NHDPlus GagesII Data",
                             theme(plot.title = 
                                     element_text(family ="Peralta", size = 40, hjust=0.5),
                                   plot.subtitle = 
                                     element_text(family="Peralta", size=24, hjust=0.5),
